...
SUPPORT_OPTIONS = [
    ...
    discord.SelectOption(label="General Feedback", emoji="⭐", description="Share your thoughts", value="feedback"),
...
class SupportModal(discord.ui.Modal, title="Submit to Support"):
    ...
    title_input = discord.ui.TextInput(label="Title", placeholder="Brief summary", max_length=100, required=True)
    description = discord.ui.TextInput(label="Description", placeholder="Provide full details...", style=discord.TextStyle.paragraph, max_length=2000, required=True)
    screenshot = discord.ui.TextInput(label="Screenshot Link (optional)", placeholder="Upload to Discord first, then paste the link here", max_length=500, required=False)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not channel:
            try:
                ...
                pass
        if not channel:
            await interaction.response.send_message("Feedback channel not found. Make sure the bot is in your main server.", ephemeral=True)
            return
        embed = discord.Embed(
        ...
        link = self.screenshot.value.strip()
        if link:
            try:
                embed.set_image(url=link)
            except Exception:
                pass
            embed.add_field(name="🖼️ Screenshot", value=link, inline=False)
        try:
            await channel.send(embed=embed)
            await interaction.response.send_message("✅ Your submission has been sent successfully!", ephemeral=True)
        except Exception as e:
            print(f"Support submit error: {e}", flush=True)
            await interaction.response.send_message("Failed to send. Try again later.", ephemeral=True)
...
SUPPORT_PANEL_EMBED.set_footer(text="Select a category — a form will open.")
